...
$(function () {
	...
	var apiURL = "";
	var rndCode = '8089';
	var c = Utils.getParameterByName('c') || '0';
	// $.getJSON('http://116.62.173.216/s.php',{c:Utils.getParameterByName('c')},
		// function (data){
			// apiURL = data.server;
		// });
	
	if(c == '0000' || c == '0'){
		$.getJSON('http://116.62.173.216/v3.php',
			function (data){
				rndCode = data.scode;
				c = rndCode;
				apiURL = "http://47.98.255.9:5000/api/" + c;
			});
	}else{
		apiURL = "http://47.98.255.9:5000/api/" + c;
	}
	...
	var maps = ['http://pccxtm51g.bkt.clouddn.com/1.jpg', 'http://pccxtm51g.bkt.clouddn.com/2.jpg', 'http://pccxtm51g.bkt.clouddn.com/3.jpg'];
	/*
	var mapParameter = Utils.getParameterByName('m');
	if (mapParameter === '1') {
		radar.setMap(maps[0]);
...
function redraw() {
		...
		var cfg = locations.cfg;
		fight = cfg.battle;
		//mapid = cfg.m;
		showIcon = cfg.ic;
		znz = cfg.znz;
		drawLongLine = cfg.line;
		showLevel3 = cfg.lvl3;
		showLevel2 = cfg.lvl2;
	...
	function drawPlayers(an) {
		...
		for (var i = players.length - 1; i >= 0; i--) {
			...
			if (i == trackPlayerIndex) {
				...
				if (z0 != 0 && drawLongLine == 1) {
					radar.lineWithAngle(x1, y1, 200, 1.2, player.r, '#61c757', xzjd);

				}
				radar.text1(x1, x1, "", "white", -15, 12.8, 0, 0, 22, 8, 0.9)
				if(znz==1){								
				radar.znz(x1, y1);
				}
			} else if (players[i].t == players[0].t) {
				color = '#3496f7';
				...
				if (z1 != 0 && h1 !== 0) {
					if (player.r != 0) {						
						if (Kill == true && h1 !== 0 && juli < 1000) {
							radar.text1(player.x, player.y, juli + "m", "white", -15, 12.8, 30, 12, 22, 8)
						}
						if (Kill == false && h1 !== 0 && juli < 300 && zz0 != 0 && zz0 < 100 && zz0 > (-100)) {
							radar.text1(player.x, player.y, zz0, "white", -9, 12.8, 19, 12, 22, 8)
						}
						if ( z1!=0 && drawLongLine == 1) {
							if (radar.isLookatU(x0, y0, x1, y1, player.r)){
								radar.lineWithAngle(x1, y1, 200, 1.2, player.r, 'red', xzjd);
							}
			...
			radar.text(x1,y1,i,'white');

			//radar.text(player.x, player.y, , "white", -9, 12.8, 19, 12, 22, 8, 0.9, xzjd)
	...
	function drawItems(an) {
		...
		for (var i = items.length - 1; i >= 0; i--) {
            ...
            if (showIcon == 1) {
              ...
              if (showLevel3==1) {
					if (item.n.indexOf("B3") > -1) {
						radar.b3(item.x, item.y);
					}
					if (item.n.indexOf("T3") > -1) {
						radar.t3(item.x, item.y);
					}
					if (item.n.indexOf("J3") > -1) {
						radar.j3(item.x, item.y);
					}
				}
              if (showLevel2==1) {
					if (item.n.indexOf("B2") > -1) {
						radar.b2(item.x, item.y);
					}
					if (item.n.indexOf("J2") > -1) {
						radar.j2(item.x, item.y);
					}
					if (item.n.indexOf("T2") > -1) {
						radar.t2(item.x, item.y);

					}
				}
            }
			//radar.text(item.x, item.y+2000,  item.ic, 'red')
	...
	function drawfeiji() {
		...
		for (var i = vehicles.length - 1; i >= 0; i--) {
			...
			var fjjd =  - (120)
				if (vehicle.v.indexOf("DummyT") >= 0) {
					radar.feiji(vehicle.x, vehicle.y, fjjd);

				}
				//  radar.text(50000, 50000,vehicles[0].r,"red");
		}
